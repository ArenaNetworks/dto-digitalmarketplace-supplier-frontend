...
if [[ ! `aws ecr describe-repositories --region $AWS_REGION --query 'repositories[*].{reponame:repositoryName}' --output text | grep ^arenanetworksmonitoring-${DOCKER_IMAGE_NAME}$` ]]; then
  echo "Docker ECR for arenanetworksmonitoring-${DOCKER_IMAGE_NAME} does not exist.  Creating"
  aws ecr create-repository --repository-name arenanetworksmonitoring-$DOCKER_IMAGE_NAME --region $AWS_REGION
fi

echo "Building $DOCKER_IMAGE_NAME"
docker build -t ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/arenanetworksmonitoring-${DOCKER_IMAGE_NAME}:${GIT_REF} $(dirname $DOCKERFILE_PATH)
echo "Pushing Docker image $DOCKER_IMAGE_NAME"
docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/arenanetworksmonitoring-${DOCKER_IMAGE_NAME}:${GIT_REF}
